...
export async function GET(_req: Request, ctx: RouteContext<"/app/[slug]/custody/[id]/custody-report.json">) {
  ...
  const row = await prisma.custodyReport.findFirst({ where: { id, orgId: org.id }, select: { report: true } });
  if (!row) return new NextResponse("not found", { status: 404 });
  return new NextResponse(row.report, {
    headers: {
      "content-type": "application/json; charset=utf-8",
      "content-disposition": 'attachment; filename="custody-report.json"',
