...
| The overview says the signing key could not be created, or the service log says `Permission denied` under `/var/lib/canton/keystore` | The keystore volume was created before the image chowned its mount point, so it is root-owned and the service (user `canton`, uid 999) cannot write to it. Newer images refuse to start in this state | `docker run --rm -v <project>_keystore:/k alpine chown 999:999 /k`, then restart the service; the console asks for the key again on the next visit to the overview |
