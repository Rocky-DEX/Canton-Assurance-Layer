...
<p align="center">
  Canton Network 交易的提交前模拟、失败原因解释与费用预估工具。<br>
  属于 <a href="../../README.zh-CN.md">Canton Assurance Layer</a> 的一部分，代码位于 <code>rust/sim-*</code>。
</p>
...
**`prepare` 看不到的部分**会写在每份报告的 caveats 里：输入合约的并发争用、对手方 participant 的包 vetting 状态、以及排序时刻的时间检查，都只在确认阶段才会验证。详见 [architecture.md](architecture.md)。
...
cargo install --path rust/sim-cli      # 安装 canton-sim
cargo install --path rust/sim-server   # 安装 canton-sim-server（可选）
```
...
请把 [`fixtures/simulator/perp-custody/`](../../fixtures/simulator/perp-custody/README.md) 中的占位合约 ID 与 party ID 替换为你 participant 上的真实值。
...
模板见 [`.env.compose.example`](../../.env.compose.example)。
...
完整的重叠分析见 [docs/grant/simulator/landscape-2026-09.md](../grant/simulator/landscape-2026-09.md)。

## 仓库结构

```
rust/sim-proto               内置的 Canton 3.4 Ledger API proto → prost 类型（protox 编译）
rust/sim-diagnose            错误目录（data/canton-error-catalog.json）、JsCantonError 解析、分类器
rust/sim-fee                 流量定价与 Splice Amulet 费用模型；Scan JSON 加载器
rust/sim-core                Ledger API 客户端、PreparedTransaction → Effects、Simulator、文本渲染
rust/sim-cli                 命令行工具
...
docs/reference/canton-dev-fund Development Fund 模板与规则快照
```
...
cargo test --workspace                          # 31 个单元与集成测试，不需要 Canton 节点
cargo clippy --workspace --all-targets -- -D warnings  # CI 中任何 warning 都会失败
cargo fmt --all
```

以上命令在 `rust/` 目录下执行；在仓库根目录运行 `scripts/check.sh rust` 会对所有 crate 跑同一套门禁。集成测试使用进程内的 axum mock 模拟 JSON Ledger API（`rust/sim-core/tests/simulate.rs`）。若要对真实 participant 测试，把 `CANTON_SIM_LEDGER_URL` 指向 LocalNet 或 DevNet validator 的 JSON API 并运行 fixtures。

为新的 Canton 版本重新生成错误目录：抽取脚本与来源规则见 [architecture.md](architecture.md#error-catalog-provenance)；每条目录条目都记录了来源文件。
...
细节与验收标准见 [docs/grant/simulator/submission/](../grant/simulator/submission/)。

## 参与贡献

欢迎贡献。请先阅读 [CONTRIBUTING.md](../../CONTRIBUTING.md) 了解工作流程、编码规范，以及如何新增目录提示或 fixture。每个 Pull Request 都必须通过 `scripts/check.sh rust`：它会对 workspace 里的每个 crate 运行 `cargo fmt`、`cargo clippy -- -D warnings`、`cargo test` 与 rustdoc。

## 社区

- 问题与功能请求：[GitHub Issues](https://github.com/Rocky-DEX/Canton-Assurance-Layer/issues)
