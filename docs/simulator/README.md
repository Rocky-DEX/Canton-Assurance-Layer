...
<p align="center">
  Pre-submit simulation, failure explanation and fee estimation for Canton Network transactions.<br>
  Part of the <a href="../../README.md">Canton Assurance Layer</a>; lives in <code>rust/sim-*</code>.
</p>

<p align="center">
  <a href="https://github.com/Rocky-DEX/Canton-Assurance-Layer/actions/workflows/ci.yml"><img src="https://github.com/Rocky-DEX/Canton-Assurance-Layer/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
...
**What `prepare` cannot see** is stated in every report's caveats: contention on input contracts, package vetting on counterparties' participants, and sequencer-time checks are only verified at confirmation. See [architecture.md](architecture.md).
...
```bash
git clone git@github.com:Rocky-DEX/Canton-Assurance-Layer.git
cd Canton-Assurance-Layer
cargo install --path rust/sim-cli      # installs `canton-sim`
cargo install --path rust/sim-server   # installs `canton-sim-server` (optional)
```
...
canton-sim simulate --act-as 'exchange::1220…' fixtures/simulator/perp-custody/platform-account-debit.json
```
...
Replace the placeholder contract and party ids in [`fixtures/simulator/perp-custody/`](../../fixtures/simulator/perp-custody/README.md) with values from your participant.
...
A template is provided in [`.env.compose.example`](../../.env.compose.example).
...
`SimulationRequest` fields: `act_as[]` (required), `commands[]` (required; JSON Ledger API `Command` objects), `read_as[]`, `user_id`, `command_id`, `disclosed_contracts[]`, `synchronizer_id`, `package_id_selection_preference[]`, `min_ledger_time_rel_secs`, `expected_signatures[]`, `lookup_contracts` (default `true`), `include_arguments` (default `true`), `include_prepared_transaction` (default `false`). A complete example is in [`fixtures/simulator/perp-custody/request-full.json`](../../fixtures/simulator/perp-custody/request-full.json).
...
The full overlap analysis is in [docs/grant/simulator/landscape-2026-09.md](../grant/simulator/landscape-2026-09.md).

## Repository Layout

```
rust/sim-proto               vendored Canton 3.4 Ledger API protos → prost types (compiled with protox)
rust/sim-diagnose            error catalog (data/canton-error-catalog.json), JsCantonError parsing, classifier
rust/sim-fee                 traffic pricing and Splice Amulet fee model; Scan JSON loader
rust/sim-core                Ledger API client, PreparedTransaction → Effects, Simulator, text renderer
...
docs/reference/canton-dev-fund snapshots of the Development Fund template and rules
```
...
cargo test --workspace                          # 31 unit and integration tests, no Canton node needed
cargo clippy --workspace --all-targets -- -D warnings  # CI fails on warnings
cargo fmt --all
```

The commands above are run from `rust/`; from the repository root, `scripts/check.sh rust` runs the same gate for every crate. Integration tests run against an in-process axum mock of the JSON Ledger API (`rust/sim-core/tests/simulate.rs`). To test against a real participant, point `CANTON_SIM_LEDGER_URL` at a LocalNet or DevNet validator's JSON API and run the fixtures.

Regenerating the error catalog for a new Canton release: the extraction script and provenance rules are described in [architecture.md](architecture.md#error-catalog-provenance); every entry records its source file.
...
Details and acceptance criteria are in [docs/grant/simulator/submission/](../grant/simulator/submission/).

## Contributing

Contributions are welcome. Please read [CONTRIBUTING.md](../../CONTRIBUTING.md) for the workflow, coding rules and how to add catalog hints or fixtures. Every pull request must pass `scripts/check.sh rust`, which runs `cargo fmt`, `cargo clippy -- -D warnings`, `cargo test` and rustdoc for every crate in the workspace.

## Community

- Issues and feature requests: [GitHub Issues](https://github.com/Rocky-DEX/Canton-Assurance-Layer/issues)
